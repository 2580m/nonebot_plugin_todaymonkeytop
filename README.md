...
- 监听 NapCat `group_msg_emoji_like` 通知，直接将事件中携带的猴子回应数量写入 SQLite；重复事件和撤销回应不会重复计数。
- 发送 `今日猴榜` 命令时直接查询 SQLite 并发送 TOP 10；命令前缀遵循 NoneBot 项目的 `command_start` 配置。
- 每日 `23:59`（`Asia/Shanghai`）向当天有消息记录的群自动发送 TOP 10。
- 记录消息映射、回应事件快照、查询与定时结算日志，便于排查。
...
- NapCat 需开启群消息事件与 `notice.group_msg_emoji_like`。

NapCat 的表情回应通知中的 `user_id` 是**贴表情的人**，而不是被贴表情的消息作者；插件会从 SQLite 中已保存的消息映射确定统计对象。未记录的消息回应会被忽略，插件不会调用 NapCat 的历史消息查询接口。
...
数据库保存 QQ 号、群号、消息 ID、显示名、日期、猴子回应数量与榜单发送标记，不保存消息正文或贴表情者列表。查榜和每日结算只读取 SQLite，不会因 NapCat 消息 ID 过期而失败。机器人重启后仍可读取当天已记录的数据；但离线期间未收到的消息或表情回应不会被补偿。插件会在每日结算后清理超过 14 天的数据。

所有插件日志写入 `data/nonebot_plugin_todaymonkeytop/log.txt`，并以 `todaymonkeytop:` 开头；其中包含每条已记录的群消息映射与回应事件快照。
